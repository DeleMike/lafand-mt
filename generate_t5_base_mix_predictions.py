...
def diacritize(text=''):
    # Load encoder model
    encoder_session = onnxruntime.InferenceSession('oyo_t5_base_mix_onnx/encoder_model.onnx')

    # Load decoder model
    decoder_session = onnxruntime.InferenceSession('oyo_t5_base_mix_onnx/decoder_model.onnx')

    model_id = "oyo_t5_base_mix_onnx"
    tokenizer = AutoTokenizer.from_pretrained(model_id)

    config = PretrainedConfig.from_json_file('oyo_t5_base_mix_onnx/config.json')

    # initialise model
    model = ORTModelForSeq2SeqLM(
        config=config,
        onnx_paths=['oyo_t5_base_mix_onnx/decoder_model.onnx','oyo_t5_base_mix_onnx/encoder_model.onnx'],
        encoder_session=encoder_session, 
        decoder_session=decoder_session, 
        model_save_dir='oyo_t5_base_mix_onnx',
        use_cache=False, 
    )
