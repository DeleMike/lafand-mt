...
def combine_texts_lafand(input_path, output_path, direction='en-sw', n_sent=50000):
    ...
    df_train = pd.read_csv(input_path + 'global_voices.tsv', sep='\t') #[For CSV, use sep=',']
    df_dev = pd.read_csv(input_path + 'dev_with_no_accents_and_underdots.tsv', sep='\t')
    ...
    export_json_files(output_dir, 'global_voices.json', df_train_sctg, direction=direction)
    export_json_files(output_dir, 'dev_with_no_accents_and_underdots.json', df_dev_sctg, direction=direction)
